...
import logging
import argparse
from models.graphllm import GraphLLM # put _nograph here
...
def upload_to_hub(args, model_path, repo_id, token):
    ...
    api.upload_file(
        ...
        path_in_repo=f"latest_checkpoint_vanilla_epoch_{args.finetune_method}_{args.llm_frozen}_v3.safetensors",
        repo_id=repo_id,
    )
        ...
        for batch_idx, batch in enumerate(progress_bar):
            try:
                ...
                if (batch_idx + 1) % args.grad_accum_steps == 0:
                    if args.grad_clip > 0:
                        ...
                        if torch.isnan(grad_norm):
                            logging.warning(f"NaN gradient norm detected at batch {batch_idx}")
                ...
                if checkpoint_interval > 0 and (batch_idx + 1) % checkpoint_interval == 0:
                    model_path = os.path.join(args.output_dir, f'latest_checkpoint_vanilla.safetensors')
                ...
                if current_step in checkpoint_points:
                    ...
                    model_path = os.path.join(args.output_dir, f'vanilla_checkpoint_{position}_of_20.safetensors')
                ...
                emergency_path = os.path.join(args.output_dir, 'emergency_checkpoint_vanilla.safetensors')
...
def test_model_saving(model, args):
    ...
    try:
        test_path = os.path.join(args.output_dir, 'test_save_vanilla.safetensors')
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='/work/hdd/bcaq/kagarwal2/vanilla')
    ...
    if args.debug:
        ...
        val_dataset = PlannerDataset(os.path.join(args.data_dir, 'vanilla_WikiofGraph_cpu.pkl'))
        ...
        data_path = os.path.join(args.data_dir, 'vanilla_WikiofGraph_cpu.pkl')
        ...
        # Define a custom map_location that forces all storages to CPU
        def force_cpu(storage, loc):
            return storage.cpu()

        # Load the data using the custom function
        # full_data = torch.load(data_path, map_location=force_cpu)
        ...
        #     while True:
        #         try:
        #             item = pickle.load(f)  # Load one item at a time
        #             # Move only the graphs back to CPU
        #             item["graphs"][0] = item["graphs"][0].cpu()
        #             full_data.append(item)  # Store it for later use
    ...
    for batch in train_loader:
        # Convert inputs and labels to tensors if necessary
        input_tensors = [torch.tensor(item) if not isinstance(item, torch.Tensor) else item for item in batch['input']]
        label_tensors = [torch.tensor(item) if not isinstance(item, torch.Tensor) else item for item in batch['label']]
        graph_tensors = [item for item in batch['graphs']]  # Assuming graphs are already tensors or PyG data

        # Check for NaN or Inf in input tensors
        for i, input_tensor in enumerate(input_tensors):
            if torch.isnan(input_tensor).any() or torch.isinf(input_tensor).any():
                raise ValueError(f"Found NaN or Inf in input data at index {i}!")

        # Check for NaN or Inf in label tensors
        for i, label_tensor in enumerate(label_tensors):
            if torch.isnan(label_tensor).any() or torch.isinf(label_tensor).any():
                raise ValueError(f"Found NaN or Inf in label data at index {i}!")

        # Check for NaN or Inf in graph tensors (if they are tensors)
        for i, graph in enumerate(graph_tensors):
            if isinstance(graph, torch.Tensor) and (torch.isnan(graph).any() or torch.isinf(graph).any()):
                raise ValueError(f"Found NaN or Inf in graph data at index {i}!")
    # Training loop
    ...
    for epoch in range(args.epochs):
        ...
        try:
            ...
            if val_loss < best_val_loss:
                ...
                model_path = os.path.join(args.output_dir, 'best_model_vanilla.safetensors')
                save_model(model, model_path)
            ...
            model_path = os.path.join(args.output_dir, 'latest_checkpoint_vanilla.safetensors')
            save_model(model, model_path)
