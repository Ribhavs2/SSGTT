...
class GraphLLM(torch.nn.Module):
    ...
    def forward(self, samples):
        ...
        if any(torch.isnan(tensor).any() for tensor in samples['input'] if isinstance(tensor, torch.Tensor)):
    ...
    def inference(self, samples):
        ...
        if torch.isnan(inputs_embeds).any() or torch.isinf(inputs_embeds).any():
            raise ValueError("Invalid values in input embeddings!")
        if torch.isnan(attention_mask).any() or torch.isinf(attention_mask).any():
            raise ValueError("Invalid values in attention mask!")


        # Generate with autocast
        with self.maybe_autocast():
        # with torch.cuda.amp.autocast(dtype=torch.float16):
        # print("used nullcontext instead")
    ...
    def maybe_autocast(self, dtype=torch.float16):  # Change to float16
